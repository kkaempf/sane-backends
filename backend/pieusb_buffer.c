/* sane - Scanner Access Now Easy.
   ...
   pieusb_buffer.c (include file for pieusb.c)
 ...
 */
static void buffer_create(struct Pieusb_Read_Buffer* buffer, SANE_Int width, SANE_Int height, SANE_Byte color_spec, SANE_Byte depth)
{
    int k, result;
    ...
    if (buffer->data_file == -1) {
        buffer->data = NULL;
	perror("buffer_create(): error opening image buffer file");
        return;
    }    
	...
	close(buffer->data_file);
        buffer->data = NULL;
	perror("buffer_create(): error calling fseek() to 'stretch' the file");
        return;
    }
	...
	close(buffer->data_file);
        buffer->data = NULL;
	perror("buffer_create(): error writing a byte at the end of the file");
        return;
    }
    /* Create memory map */
    ...
    if (buffer->data == MAP_FAILED) {
        close(buffer->data_file);
        buffer->data = NULL;
        perror("buffer_create(): error mapping file");
        return;
    }
    ...
    DBG(DBG_info,"Read buffer created: w=%d h=%d ncol=%d depth=%d in file %s\n",
...
static SANE_Int buffer_put_single_color_line(struct Pieusb_Read_Buffer* buffer, SANE_Byte color, void* line, SANE_Int size)
{
    ...
    DBG(DBG_info,"buffer_put_single_color_line() entered\n");
    ...
    if (c == -1) {
        DBG(DBG_error, "buffer_put_single_color_line(): color '%c' not specified when buffer was created\n", color);
        return 0;
    }
    DBG(DBG_info,"buffer_put_single_color_line() line color = %d (0=R, 1=G, 2=B, 3=I)\n",c);
    
    /* Check line size (for a line with a single color) */
    if (buffer->line_size_bytes != size) {
        DBG(DBG_error, "buffer_put_single_color_line(): incorrect line size, expecting %d, got %d\n", buffer->line_size_bytes, size);
        return 0;
    }
    ...
    if (buffer->packet_size_bytes == 1 && buffer->packing_density == 1) {
        ...
        uint8_t* p_packet = (uint8_t*)line;
        uint8_t packet[buffer->packet_size_bytes];
        SANE_Uint val;
...
static SANE_Int buffer_put_full_color_line(struct Pieusb_Read_Buffer* buffer, void* line, int size)
{
    ...
    DBG(DBG_info,"buffer_put_full_color_line() entered\n");
    
    /* Check line size */
    if (buffer->line_size_bytes * buffer->colors != size) {
        DBG(DBG_error, "buffer_put_full_color_line(): incorrect line size, expecting %d, got %d\n", buffer->line_size_bytes * buffer->colors, size);
        return 0;
    }

/* sane - Scanner Access Now Easy.
   ...
   pieusb_specific.c (include file for pieusb.c)
...
pieusb_supported_device_list_add(SANE_Word vendor_id, SANE_Word product_id, SANE_Word model_number)
{
    ...
    for (k=0; k<=i+1; k++) {
        DBG(DBG_info_proc,"pieusb_supported_device_list_add() add: %03d: %04x %04x %02x\n", i,
            pieusb_supported_usb_device_list[k].vendor,
            pieusb_supported_usb_device_list[k].product,
            pieusb_supported_usb_device_list[k].model);
    }
...
static SANE_Status pieusb_get_shading_data(Pieusb_Scanner * scanner)
{
    ...
    switch (scanner->mode.colorFormat) {
        case SCAN_COLOR_FORMAT_PIXEL: /* Pixel */
            ...
            cmdGetScannedLines(scanner->device_number, buffer, shading_height*4, (2*shading_width)*shading_height*4, &status, 5);
            break;
        case SCAN_COLOR_FORMAT_INDEX: /* Indexed */
            buffer = malloc((2*shading_width+2)*shading_height*4);
            cmdGetScannedLines(scanner->device_number, buffer, shading_height*4, (2*shading_width+2)*shading_height*4, &status, 5);
            break;
        default:
...
static SANE_Status pieusb_get_scan_data(Pieusb_Scanner * scanner)
{
    struct Pieusb_Command_Status status;
    struct Pieusb_Scan_Parameters parameters;
    SANE_Int lines_to_read, lines_to_store, lines_read;
    SANE_Int ppl, bpl;
...
*/
    while (lines_read < lines_to_read) {
        ...
        if (parameters.availableLines > 0) {
            /* Read lines */
...
*/
             ...
             * overflow the buffer. */
            lines_to_store = (lines_to_read - lines_read >= parameters.availableLines) ? parameters.availableLines : lines_to_read - lines_read;
            lboff = linebuf;
            switch (scanner->mode.colorFormat) {
                case SCAN_COLOR_FORMAT_PIXEL:
                    ...
                    compress = SANE_FALSE;
                    if (scanner->buffer.colors == 1 && bpl*scanner->buffer.packing_density/ppl == 3*scanner->buffer.packet_size_bytes) {
                        compress = SANE_TRUE;
                    }
            ...
            free(linebuf);
            lines_read += parameters.availableLines; /* Note: excess discarded */

...
{   IFD *ifd;
    ...
    ifd->ifde = malloc (maxtags * sizeof (IFD_ENTRY));
    if (ifd->ifde == NULL)
    {
        free (ifd);
        return NULL;
...
{IFD *ifd;
    ...
    if (icc_profile)
    {
      ...
      if (!icc_file)
      {
        DBG(DBG_error, "Could not open ICC profile %s\n", icc_profile);
      }
      else
      {
    ...
    if (icc_len > 0)
    {
      ...
      for (i=0; i<icc_len; i++)
      {
        ...
        {
          DBG(DBG_error, "ICC profile %s is too short\n", icc_profile);
          break;
        }
      }
    }
...
/**
 * Four-channel TIFF header, to export I next to RGB.
 * Straightforward adaptation of write_tiff_color_header().
 * 
 * @param fptr File to write to
 * @param width Image width (number of pixels)
 * @param height Image height (number of pixels)
 * @param depth Image depth (number of bits for a single color)
 * @param resolution Image resolution (pixels per inch)
 * @param icc_profile
 */
static void
